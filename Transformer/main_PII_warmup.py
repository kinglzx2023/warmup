...
parser.add_argument('--save', type=str, default='model_PII_1.pt',
...
name = 'warmup_'+ 'adam' +\
        ...
        '_emsize:' +str(args.emsize) +\
        '_nhid:' +str(args.nhid) +\
        '_nlayers:' + str(args.nlayers) +\
        '_nhid:'+ str(args.nhid)+\
        '_nlayers:'+str(args.nlayers)
...
def cos_similarity_atten(index,block):
    ...
    print(mean_cos_sim_row, mean_cos_sim_column)
    file1.writelines(str(Matrix[index])+':'+str(mean_cos_sim_row)+','+ str(mean_cos_sim_column)+'  ')
...
scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps = 20, num_training_steps =args.epochs )
    ...
    for batch, i in enumerate(range(0, train_data.size(0) - 1, args.bptt)):
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            for name, param in model.named_parameters():
                if name == 'encoder.layers.3.self_attn.in_proj_weight':
                    atten_blocks = column_block_matrix(param.cpu().data, 500)
                    for index,block in enumerate(atten_blocks):
                        cos_similarity_atten(index,block)
                    file1.writelines('\n')
